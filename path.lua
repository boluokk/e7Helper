...
path.刷书签 = function (rest)
	...
	if current_task['神秘奖牌'] then table.insert(target, '国服神秘商店神秘奖牌') end
	if current_task['誓约书签'] then table.insert(target, '国服神秘商店誓约书签') end
			...
			for i=1,4 do
				...
				wait(function ()
					...
					if r2 == '国服神秘商店立即更新' then
						-- 统计获得物品次数
						if countTarget then
							if countTarget == '国服神秘商店神秘奖牌' then
								g1 = g1 + 1
								setNumberConfig("g1", g1)
							elseif countTarget == '国服神秘商店誓约书签' then
								g2 = g2 + 1
...
path.战斗跑图1 = function (typeTarget, levelTarget, fightCount)
	local p
	...
	if not wait(function ()
		return wait(function ()
			if findTapOnce('国服短选择队伍') then
				return wait(function ()
					if not findOne('国服短选择队伍') then return 1 end
				end, .5, 5)
			end
		end)
	end, .5, 3) then
...
-- 834,686 834,147
-- fightCount: 10
path.通用刷图模式1 = function (fightCount)
	untilAppear('国服右下角', {keyword = {'战斗开始'}})	ssleep(.5)
	-- 这里如果有的话,就处理
	-- local isAgent = path.托管处理()
	local pos, noAct
	if wait(function ()
		pos, noAct = findOne({'国服背包空间不足', '国服行动力不足', '国服右下角'}, {keyword = {'战斗开始', '选择队伍'}})
		...
		if noAct == '国服右下角' then stap({pos[1].l, pos[1].t}) end
...
path.托管处理 = function ()
	...
	local isAgent
	if not wait(function ()
		greenPos = findOne('国服是否可自动挂机', {rg = {563,528,685,584}, sim = .9})
